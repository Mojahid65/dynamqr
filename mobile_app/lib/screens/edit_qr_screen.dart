...
class _EditQrScreenState extends State<EditQrScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _qrType = widget.qrData['qr_type'] ?? 'url';
    _isPasswordProtected = widget.qrData['is_password_protected'] ?? false;
    
    if (widget.qrData['scan_limit'] != null) {
      _scanLimitController.text = widget.qrData['scan_limit'].toString();
    }
    
    if (widget.qrData['expires_at'] != null) {
      _expiresAt = DateTime.tryParse(widget.qrData['expires_at']);
    }

    _expirationMessageController.text = widget.qrData['expiration_message'] ?? '';
    _expirationUrlController.text = widget.qrData['expiration_url'] ?? '';

    // Populate Type Data
    final typeData = widget.qrData['type_data'] as Map<String, dynamic>? ?? {};
    if (_qrType == 'wifi') {
      _wifiSsidController.text = typeData['ssid'] ?? '';
      _wifiPasswordController.text = typeData['password'] ?? '';
      _wifiEncryption = typeData['encryption'] ?? 'WPA';
      _wifiHidden = typeData['hidden'] ?? false;
    } else if (_qrType == 'vcard') {
      _vFirstNameController.text = typeData['firstName'] ?? '';
      _vLastNameController.text = typeData['lastName'] ?? '';
      _vPhoneController.text = typeData['phone'] ?? '';
      _vEmailController.text = typeData['email'] ?? '';
      _vCompanyController.text = typeData['company'] ?? '';
      _vTitleController.text = typeData['title'] ?? '';
      _vWebsiteController.text = typeData['website'] ?? '';
      _vAddressController.text = typeData['address'] ?? '';
    } else if (_qrType == 'email') {
      _emailToController.text = typeData['email'] ?? '';
      _emailSubjectController.text = typeData['subject'] ?? '';
      _emailBodyController.text = typeData['body'] ?? '';
    } else if (_qrType == 'sms') {
      _smsPhoneController.text = typeData['phone'] ?? '';
      _smsMessageController.text = typeData['message'] ?? '';
    }

    // Populate Design Config
    final config = widget.qrData['design_config'] as Map<String, dynamic>?;
    if (config != null) {
      _qrTheme = config['dotType'] ?? config['theme'] ?? 'square';
      _bgType = config['bgType'] ?? 'color';
      _bgImageUrlController.text = config['bgImage'] ?? '';
      _bgOpacity = (config['bgOpacity'] as num?)?.toDouble() ?? 0.85;

      if (config['fgColor'] != null) _qrColor = _hexToColor(config['fgColor']) ?? Colors.black;
      if (config['color'] != null) _qrColor = _hexToColor(config['color']) ?? Colors.black;
      if (config['eyeColor'] != null) _qrEyeColor = _hexToColor(config['eyeColor']) ?? Colors.black;
      if (config['eye_color'] != null) _qrEyeColor = _hexToColor(config['eye_color']) ?? Colors.black;
      if (config['bgColor'] != null) _qrBgColor = _hexToColor(config['bgColor']) ?? Colors.white;

      _frameId = config['frameId'] ?? 'none';
      _logoType = config['logoType'] ?? 'none';
      _logoPresetId = config['logoPresetId'] ?? 'link';
  ...
  Future<void> _updateQrCode() async {
    ...
    try {
      ...
      final designConfig = {
        ...(widget.qrData['design_config'] as Map<String, dynamic>? ?? {}),
        'theme': _qrTheme,
        'dotType': _qrTheme,
      ...
      final updates = <String, dynamic>{
        ...
        'scan_limit': _scanLimitController.text.isNotEmpty ? int.tryParse(_scanLimitController.text) : null,
      ...
      await Supabase.instance.client.from('qr_codes').update(updates).eq('id', widget.qrData['id']);

      if (mounted) {
        messenger.showSnackBar(const SnackBar(content: Text('QR Code updated successfully!')));
        context.pop(true);
      }
    } catch (e) {
      if (mounted) {
          ...
          if (_isPasswordProtected)
            Padding(
              ...
              child: TextFormField(
                ...
                obscureText: true,
                decoration: const InputDecoration(
                  labelText: 'Set Password (leave blank to keep current)',
            ...
            onTap: () async {
              ...
              if (date != null) {
                final time = await showTimePicker(
                  context: context,
                  initialTime: _expiresAt != null ? TimeOfDay.fromDateTime(_expiresAt!) : TimeOfDay.now(),
                );
                if (time != null) {
                  setState(() {
